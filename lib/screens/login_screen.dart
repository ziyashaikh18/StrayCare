...
class _LoginScreenState extends State<LoginScreen> {
  ...
  Future<void> _handleGoogleSignIn() async {
    ...
    debugPrint("[GoogleSignIn] Button clicked.");

    try {
      // Ensure previous session doesn't block re-selecting account if needed
      await _googleSignIn.signOut().catchError((_) => null);

      debugPrint("[GoogleSignIn] Google account picker opening...");
      final GoogleSignInAccount? googleUser = await _googleSignIn.signIn();

      if (googleUser == null) {
        debugPrint("[GoogleSignIn] User cancelled Google Sign-In.");
        if (mounted) {
          ScaffoldMessenger.of(context).showSnackBar(
      ...
      debugPrint("[GoogleSignIn] Google account picker selected: ${googleUser.email}");
      final GoogleSignInAuthentication googleAuth = await googleUser.authentication;
      final String? idToken = googleAuth.idToken;

      if (idToken == null || idToken.isEmpty) {
        debugPrint("[GoogleSignIn] Error: Google ID token is null or empty.");
        if (mounted) {
          ScaffoldMessenger.of(context).showSnackBar(
      ...
      debugPrint("[GoogleSignIn] ID token received. Sending to backend: ${ApiConfig.baseUrl}/api/auth/google");
      final response = await http.post(
      ...
      if (response.statusCode == 200 && data["success"] == true) {
        ...
        debugPrint("[GoogleSignIn] Navigation completed. Role: $role");
        if (role == "admin") {
          Navigator.pushReplacement(
            context,
            MaterialPageRoute(
        ...
        } else if (role == "reporter" || role == null || role.isEmpty) {
          Navigator.pushReplacement(
            context,
            MaterialPageRoute(
              builder: (_) => const HomeScreen(),
        ...
        if (!mounted) return;
        debugPrint("[GoogleSignIn] Backend failed: ${data["message"]}");
        ScaffoldMessenger.of(context).showSnackBar(
          SnackBar(
      ...
      debugPrint("[GoogleSignIn] Network/Execution error: $e");
      if (!mounted) return;
      ScaffoldMessenger.of(context).showSnackBar(
        SnackBar(
